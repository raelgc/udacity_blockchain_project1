...
class Blockchain {
    ...
    submitStar(address, message, signature, star) {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            if (currentTime - messageTime < 5*60*60) {
                if (bitcoinMessage.verify(message, address, signature)) {
                    const block = new BlockClass.Block({ star, address });
                    self._addBlock(block).then(resolve(block));
                } else {
                    reject('Verification failed');
                }    
            } else {
                reject('Time is higher than 5 minutes');
            }
        });
    }
    ...
    getBlockByHash(hash) {
        let self = this;
        return new Promise((resolve, reject) => {
           if (block = self.chain.find(b => b.hash === hash)){
               resolve(block);
           } else {
    ...
    getStarsByWalletAddress (address) {
        let self = this;
        return new Promise((resolve, reject) => {
            const stars = self.chain.reduce((acc, b) => {
                try {
